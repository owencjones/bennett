...
from optool import retrieve_api_output
from optool.exceptions import OPToolException_API_connection_failed
from optool.open_prescribe import retrieve_single_drug
...
class TestApiRetrieveApiOutput:
    ...
    def get_http_fixture(self):
        with patch("optool.open_prescribe.get_http", spec=Response) as get_http_mock:
            ...
            get_http_mock.json.return_value = {
                "foo": "bar"
            }

            yield get_http_mock

    @pytest.mark.parametrize("status_code", list(range(200, 600)))
    def test_all_of_the_valid_statuses(self,status_code: int, get_http_fixture: Response):
...
class TestRetrieveSingleDrug:
    ...
    @pytest.mark.parametrize("bnf_code, expectation", [
